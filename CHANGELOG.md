...
### Added
- 竿頭の推移グラフ（直近14日、船ごとの折れ線、依存ライブラリなしのSVG）。魚種チップで絞ると、その魚種の max を描く。点をタップすると船名・日付・匹数
- 「船ごとの最新」で魚種フィルタ中は、その魚種を数値なしでも表示（他魚種で埋めない）

## [0.2.1] - 2026-09-13

### Changed
- 抽出プロンプト修正：全角数字の数値化、「30パイ越え／超／以上」→ max=30、「多い方で15匹程」を竿頭として扱う、top_count は主ターゲットの max と一致させる
  - 背景：大海丸 9/1 記事の「トップは３０パイ越え」が匹数として拾えていなかった
...
### Not yet
- 既存データの再抽出（0.2.1 のプロンプトは新着記事にのみ適用される）
