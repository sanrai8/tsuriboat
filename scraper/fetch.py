...
- 匹数は「〜杯」「〜ハイ」「〜パイ」「〜匹」「〜本」「〜枚」を数値化。「5〜30杯」なら min=5 max=30。
  「トップ39ハイ」「竿頭39杯」のように竿頭だけ書かれていれば max にその値、min は null。
- 全角数字（３０パイ、１５匹）も必ず半角の数値にする。
- 「30パイ越え」「30杯超」「30杯以上」は max=30 とする（越えた分は無視）。
- 「船中436ハイ」「合計47本」のような合計は total に入れる。
- 「多い方で15匹程」「多い人で20杯」も竿頭の表現なので max に入れる。
- top_count は主ターゲット（記事の主役の魚種）の max と同じ値を入れる。
"""
